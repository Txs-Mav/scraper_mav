...
export function inferVehicleType(product: { sourceUrl?: string; name?: string }): VehicleType {
  ...
  if (/\/cotes?-a-cotes?|\/side-by-side/.test(url)) return "cote-a-cote"
  ...
  if (/\/equipement-mecanique|\/produits-mecaniques|\/power-equipment/.test(url)) return "equipement"
  ...
  if (/\b(?:teryx|mule|uforce|zforce|maverick|ranger|rzr|defender|ridge)\b/.test(name)) return "cote-a-cote"
